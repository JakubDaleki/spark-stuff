...
// declaring a function to get timestamp of current local time
...
def upsertDeltaLake(df: DataFrame, dataDir: String): Unit = {
...
// declaring a upsert logic. Please note business id columns can change in the future e.g. new_df.Id = t.Id
def upsert(df: DataFrame, batchId: Long, tableName: String, dataDir: String): Unit = {
  val dfDeduplicated = deduplicateDf(df)
  dfDeduplicated.persist()
  val isInitalized = DeltaTable.isDeltaTable(spark, dataDir)
  if (isInitalized) {
    upsertDeltaLake(dfDeduplicated, dataDir)
  } else {
...
def writeAutoloaderDf(df: DataFrame, silverDeltaLocation: String, checkpointLocation: String, tableName: String): StreamingQuery = {
  ...
  val query = df.writeStream
      ...
      .foreachBatch{(batchDF: DataFrame, batchId: Long) => 
         upsert(batchDF, batchId, tableName, silverDeltaLocation)}
...
// function responsible for reading new files from bronze location and writing it to silver location. In case of existance of the tables the upsert is performed.
def deltaLakeInsert(tableName: String): StreamingQuery = {
...
def insertionSteps(table: String): String = {
  val stream = deltaLakeInsert(table)
  // if UnknownFieldException was thrown, this command will fail with StreamingQueryException
  stream.awaitTermination()
  // this will print progress and logs of upserting for each table
